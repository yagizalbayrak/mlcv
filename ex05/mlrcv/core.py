...
def plot_model(x, y, model):
    ...
    x1_step = (x1max - x1min) / 30.0
    x2_step = (x2max - x2min) / 30.0
        ...
        for i in range(30):
            for j in range(30):
                ...
                if grid_pred_.shape[-1] != 1:
                    grid_pred[0] += np.array([1.0, 0.0, 0.0]) * grid_pred_[0][0]
                    grid_pred[0] += np.array([0.0, 1.0, 0.0]) * grid_pred_[0][1]
                    grid_pred[0] += np.array([0.0, 0.0, 1.0]) * grid_pred_[0][2]
                    grid_pred[0] += np.array([1.0, 1.0, 0.0]) * grid_pred_[0][3]
                else:
                    if grid_pred_[0] != 3:
                        grid_pred[0, int(grid_pred_[0])] = 1.0
                    else:
                        grid_pred[0, 0] = 1.0
                        grid_pred[0, 1] = 1.0
    ...
    plt.scatter(x[cls4, 0], x[cls4, 1], color=(1.0, 1.0, 0.0, 1.0))
...
def split_data(x, y):
    ...
    train_index = np.random.choice(len(x), int(len(x) * 0.7), replace=False)
