...
class CenterNetTrainer(pl.LightningModule):
    def __init__(self, model, criterion, loss_weights, train_loader, val_loader, lr, epochs):
        ...
        self.train_step = 0
        self.val_step = 0
        self.predictions_sizemap = []
        self.predictions_heatmap = []
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch % 150 or self.current_epoch == self.epochs - 1:
            self.predictions_heatmap.append(pred_htm.cpu())
            self.predictions_sizemap.append(pred_szm.cpu())
    ...
    def on_validation_epoch_end(self):
        if self.current_epoch % 150 or self.current_epoch == self.epochs - 1:
            ...
            heatmap = np.concatenate(self.predictions_heatmap, axis=0)[:10]
            sizemap = np.concatenate(self.predictions_sizemap, axis=0)[:10]
            np.savez("predictions.npz", heatmap=heatmap, sizemap=sizemap)
